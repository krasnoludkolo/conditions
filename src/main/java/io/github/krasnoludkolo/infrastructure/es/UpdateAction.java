...
final class UpdateAction<T extends Identifiable<Integer>> implements Command<Repository<T>, Success>, Serializable {

    private static final long serialVersionUID = 1L;

    private final T toUpdate;

    UpdateAction(T toUpdate) {
        this.toUpdate = toUpdate;
    }

    @Override
    public Success execute(Repository<T> system) {
        return system.update(toUpdate);
    }
}
